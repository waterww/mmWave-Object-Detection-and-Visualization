...
void *DataUARTHandler::sortIncomingData( void )
{
    ...
    boost::shared_ptr<pcl::PointCloud<pcl::PointXYZINormal>> RScan(new pcl::PointCloud<pcl::PointXYZINormal>);
        ...
        case READ_OBJ_STRUCT:
            ...
            while( i < mmwData.numObjOut ) {
                if (((mmwData.header.version >> 24) & 0xFF) < 3) { // SDK version is older than 3.x
                    ...
                    //get object x-coordinate expected ‘;’ before ‘DataUARTHandler’
                    ...
                    RScan->points[i].curvature = temp[7];//velocity
                    ...
                    RScan->points[i].curvature = mmwData.newObjOut.velocity;
